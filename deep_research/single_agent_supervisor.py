...
async def solve_simple_task(state: AgentState, writer=None) -> dict:
    ...
    if route == "RETRIEVE" or task_type == "kb_retrieval":
        system_instruction += (
            ...
            "4. 「不要硬推断」仅指：不得把知识库未明确写出的具体条款、数值、现场结论捏造为“来自知识库”；"
            "不得把仅关键词相关的片段曲解为已回答用户问题。"
            "5. 若检索结果为空、为系统提示「未能检索到」、或与用户问题主题不匹配（仅有词面重叠）："
            "须先用一两句话说明知识库侧情况；随后必须用你已掌握的通用专业知识补全用户问题的可答部分，"
            "并在【逻辑分析】中用单独小段标题「知识库依据」与「通用知识补充」区分两类内容；"
            "通用知识部分不要伪称为某条检索原文。"
            "6. 若知识库证据足以覆盖问题主体，以证据为主作答，通用知识仅作必要衔接，可不单列「通用知识补充」。"
        )
    ...
    if route == "RETRIEVE" or task_type == "kb_retrieval":
        closing = (
            "优先依据上述检索证据作答与证据直接相关的部分；"
            "若证据缺失、无关或不足以支撑用户问题的核心，须说明知识库情况后，用通用专业知识补全回答，"
            "并在【逻辑分析】中区分「知识库依据」与「通用知识补充」。"
            "生成一份自包含、直接可读的回答。"
        )
    else:
        closing = (
