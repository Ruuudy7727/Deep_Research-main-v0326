...
async def api_stream():
    async def event_generator():
        ...
        while True:
            ...
            elapsed_seconds = SHARED_STATE.get("last_answer_elapsed_seconds")
            steps_hash = json.dumps(steps, ensure_ascii=False, default=str)
            if steps_hash != last_steps_hash or elapsed_seconds != last_elapsed_sent:
                last_steps_hash = steps_hash
                last_elapsed_sent = elapsed_seconds
                yield f"event: timeline\ndata: {json.dumps({'steps': steps, 'elapsed_seconds': elapsed_seconds}, ensure_ascii=False)}\n\n"
